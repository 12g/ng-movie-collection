...
export abstract class DataInMemoryService<T extends AbstractEntity>
      ...
      if (filter.hasOwnProperty(propName) && propName !== 'id') {
        ...
        if (typeof propValue === 'string') {
          matchingItems = matchingItems.filter(
            it => propName in it &&
                  (it[propName] as string).toUpperCase().includes(propValue.toUpperCase()));
          ...
          if (propValue instanceof Date) {
            ...
            matchingItems = matchingItems.filter(
              it => propName in it &&
                    (it[propName] as AbstractEntity).id === propValue.id);
          }
        }
      }
    }
